...
import 'package:flutter_test_module/widget/video_list/server_data.dart';
import 'package:flutter_test_module/widget/video_list/model/video_model.dart';
...
abstract class VideoController {
  ...
  Future<void> init() async {
    // 首先判断一级缓存——即内存中是否有数据
    print('FlutterModule- init video controller');
    if (dataList == null) {
      print('FlutterModule- model is null');
      // 如果没有，则从二级/三级缓存查找
  ...
  Future<List<VideoModel>> fetchVideoData() async {
    ...
    if (modelStr != null && modelStr.isNotEmpty) {
      // 二级缓存中找到数据，直接使用
      print('FlutterModule- 2/use sp data');
      ...
      print('FlutterModule- 3/fetch data from server');
